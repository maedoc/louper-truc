...
function transition(event) {
  ...
  if (!VALID_STATES.has(current)) { s.interaction = 'idle'; return; }
...
function updateSelection(x) {
  ...
  else { s.loopEnd = s.loopStart; s.loopStart = ti; }
}

function finalizeSelection() {
  if (s.loopEnd - s.loopStart < LOOP_MIN_SEC) { s.loopStart = 0; s.loopEnd = 0; }
